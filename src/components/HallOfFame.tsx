...
import { ChevronDown, ChevronRight, MapPin, Award, BookOpen, Search, TrendingUp } from 'lucide-react';
  ...
  const fetchSubmissions = async () => {
    try {
      const [submissionsData, katasData, kataSetsData, latestSubmissionsData, countriesData] = await Promise.all([
        supabase
          .from('submissions')
          .select('id, country, email, youtube_url, video_file_path, level, participant_names, message, submitted_at, dojos(name, country)')
          .eq('status', 'approved')
          .order('submitted_at', { ascending: false }),
        supabase.from('katas').select('*').order('order_index'),
        supabase.from('kata_sets').select('*').order('set_number'),
        supabase.from('submissions').select('id, youtube_url, video_file_path, level, submitted_at, country, dojos(name, city)').eq('status', 'approved').order('submitted_at', { ascending: false}).limit(12),
            ...
            <div className="relative z-10">
              ...
              <div className="max-w-3xl mx-auto text-center">
                ...
                <button
                  ...
                  className="text-xl font-bold text-yellow-300 hover:text-yellow-200 transition-colors duration-300 cursor-pointer"
                >
                          ...
                          {countrySubmissions.map((submission) => {
                            ...
                            return (
                              <div key={submission.id} className="group">
                                <a
                                  href="https://www.youtube.com/@TripleWazaFriendshipChallenge"
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="block relative aspect-video rounded-xl overflow-hidden w-full shadow-md hover:shadow-2xl transition-all duration-300 hover:scale-105"
                                >
                                  {videoId ? (
                                    <>
                                      <img
